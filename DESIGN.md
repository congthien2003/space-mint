...
Cursor's marketing site reads as a quietly-confident developer brand that believes in editorial calm over IDE-darkness. The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand voltage is **Crimson Pulse** (`{colors.primary}` — #e53935) reserved for primary CTAs and the wordmark — used scarcely.
...
- Single CTA color: `{colors.primary}` (Crimson Pulse #e53935). Used scarcely.
...
- **Crimson Pulse** (`{colors.primary}` — #e53935): Primary CTA pills, wordmark, hero accent. Used scarcely. A vivid, energetic red that signals action without losing the editorial calm.
- **Crimson Pulse Active** (`{colors.primary-active}` — #c62828): Press state.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Crimson Pulse.
...
**`button-primary`** — The signature Crimson Pulse CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 500), padding 10px × 18px, height 40px, rounded `{rounded.md}` (8px).
...
**`cta-band`** — Pre-footer "Try Cursor now" band. Background `{colors.canvas}`, centered display headline in `{typography.display-lg}`, single Crimson Pulse CTA. 96px vertical padding.
...
- Reserve `{colors.primary}` (Crimson Pulse) for primary CTAs and brand wordmark.
...
- Don't introduce a secondary brand action color. Crimson Pulse is the only one.
